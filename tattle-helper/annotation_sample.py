...
data = list(
    coll.aggregate(
        [
            {
                "$match": {
                    "$and": [{"language": "Hindi"}, {"media_type": {"$ne": "link"}}]
                }
            },
            {"$sample": {"size": 50}},
        ]
...
for dataobj in data:
    ...
    metadata = {
        "_id": str(dataobj["_id"]),
        "media_type": dataobj.get("media_type", ""),
        "post_permalink": dataobj.get("post_permalink", ""),
        "tag_name": dataobj.get("tag_name", ""),
        "s3_url": dataobj.get("s3_url", ""),
        "timestamp": str(dataobj["timestamp"]),
        "caption": dataobj.get("caption", ""),
